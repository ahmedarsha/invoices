							...
							<div class="menu-header-content bg-primary text-right">
								<div class="d-flex">
									...
									<span class="badge badge-pill badge-warning mr-auto my-auto float-left">
										<a href="{{route('invoices.markAsRead')}}"> تعين الكل مقروء</a>
									</span>
								</div>
								<p class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12 ">
									لديك <span style="color: yellow" id="notifications_count">{{ auth()->user()->unreadNotifications->count() }}</span>  من الاشعارات غير المقروءه </p>
							</div>
							<div class="main-notification-list Notification-scroll" id="unreadNotifications">
								@foreach(auth()->user()->unreadNotifications as $notification)
									<a class="d-flex p-3" href="{{ route('invoicesDetails',$notification->data['id']) }} ?notfyid={{ $notification->id }}">
